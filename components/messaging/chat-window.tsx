...
export function ChatWindow({ bookingId, hostName, hostAvatar }: ChatWindowProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      content: "Hi! Welcome to my property. Let me know if you have any questions.",
        ...
        <div className="space-y-4">
          {messages.map((message) => (
            <div key={message.id} className={`flex gap-3 ${message.isCurrentUser ? "flex-row-reverse" : ""}`}>
              ...
              <div className={`max-w-[70%] space-y-1 ${message.isCurrentUser ? "items-end" : ""}`}>
                <div
                ...
                <p className="px-1 text-xs text-muted-foreground">{format(new Date(message.timestamp), "h:mm a")}</p>
              </div>
            </div>
          ))}
